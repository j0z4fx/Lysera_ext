...
3. Run it. The loader checks `release/version.txt`, downloads changed runtime files, verifies every SHA-256 checksum, and launches `Lysera.exe` as administrator.
...
Replace the files under `release/`, regenerate `release/checksums.txt`, and change `release/version.txt`. Existing loader installations will update on their next launch.
